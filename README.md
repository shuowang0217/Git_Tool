...
## 2026-07-07 v1.5.1 更新说明

本版本主要同步程序版本号、打包名称和说明文档，避免源码已经是 `v1.5.1`，但 README、打包脚本仍显示 `v1.5`。

本次调整：

1. 程序版本号为 `APP_VERSION = "1.5.1"`。
2. 打包输出名称改为 `Git-Tool-v1.5.1.exe`。
3. README 和优化说明同步更新到 `v1.5.1`。
4. 保留默认主分支 `master` 的初始化逻辑。
5. 删除旧的 `Git-Tool-v1.5.spec`，避免误用旧版本打包配置。
